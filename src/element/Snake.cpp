...
// 初始化最高分
int sfSnake::Snake::highestScore = 0;
...
void Snake::grow(int score)
{
    if(score == -2)
    {
        dieSound_.play(); // 播放死亡音效
        sf::sleep(sf::seconds(dieBuffer_.getDuration().asSeconds())); // 暂停以播放完整音效
        hitSelf_ = true; // 触发自我碰撞，游戏结束
    }
    if(score == -3)
    {
        sfSnake::Snake::highestScore = 0;
    }

    // 为了避免在碰撞检测时误判，将分数乘以10
    if(score >= 0)
    {
